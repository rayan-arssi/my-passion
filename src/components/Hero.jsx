...
function Line({ text, index }) {
  return (
    <motion.div
      className={`hero-line${index === 1 ? ' hero-line--brush' : ''}`}
      initial="hidden"
      animate="visible"
      variants={{
        hidden: {},
...
export default function Hero() {
  ...
  const brushX = useTransform(smx, (v) => v * -55);
  const brushY = useTransform(smy, (v) => v * -30);
    ...
    <section ref={sectionRef} className="hero" id="top" onMouseMove={onMove}>
      ...
      <div className="hero-top">
        <div className="hero-mast">
          ...
          >
            DIGITAL DESIGNER <span className="hero-dot">·</span> WEB &amp; FRONTEND DEVELOPER
          </motion.p>
        </div>
        <motion.div
      ...
      >
        ...
        <svg className="hero-scroll-arrow" width="18" height="34" viewBox="0 0 18 34" fill="none" aria-hidden="true">
          <path d="M9 1 L9 31 M2 24 L9 32 L16 24" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </motion.div>
