...
function WorkRow({ project, onOpen }) {
  const rowVariants = {
    ...
    visible: {
      opacity: 1,
      y: 0,
      transition: { duration: 0.8, ease, staggerChildren: 0.07, delayChildren: 0.08 },
    },
      ...
      <div className="work-info">
        ...
        <motion.div className="work-tags" variants={itemVariants}>
          {project.stack.map((t, i) => (
            <span key={`${t}-${i}`} className="work-pill">{t}</span>
          ))}
          <span className={`work-status work-status--${project.statusClass}`}>{project.status}</span>
        </motion.div>
        ...
        <div className="modal-meta">
          ...
          <div className="modal-stack">
            {project.stack.map((s, i) => (
              <span key={`${s}-${i}`} className="modal-pill">{s}</span>
            ))}
          </div>
